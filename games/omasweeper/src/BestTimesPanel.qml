...
OmaPanel {
    ...
    width: Math.min(parent.width - 48 * theme.textScale, 640 * theme.textScale)
    height: column.implicitHeight + 2 * padding

    function entriesFor(id) {
        return game.bestTimes.filter(function(entry) { return entry.preset === id; });
    }

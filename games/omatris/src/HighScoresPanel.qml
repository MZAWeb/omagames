...
OmaPanel {
    ...
    function entriesFor(id) {
        return game.highScores.filter(function(entry) { return entry.mode === id; });
    }
        ...
        RowLayout {
            ...
            Repeater {
                ...
                ColumnLayout {
                    ...
                    readonly property bool byTime: modelData.goal > 0
                    ...
                    Repeater {
                        ...
                        RowLayout {
                            ...
                            Text {
                                Layout.fillWidth: true
                                text: table.byTime ? clock.text(row.modelData.millis)
                                                   : row.modelData.score.toLocaleString(Qt.locale(), "f", 0)
                                color: theme.foreground
                                font.pixelSize: 13 * theme.textScale
                                font.bold: row.index === 0
                            }
                            Text {
                                text: table.byTime ? row.modelData.score.toLocaleString(Qt.locale(), "f", 0)
                                                   : qsTr("L%1").arg(row.modelData.level)
